...
from eval_models.magiclens.layers import AttenTokenPoolingLayer
from eval_models.magiclens.layers import StackedTransformer
...
class MagicLens(nn.Module):
  ...
  def clip_encode(self, input_batch: Dict) -> Tuple[jax.Array, jax.Array]:
    ...
    if 'image' in input_batch:
      ...
      text_embs = self.clip.encode_text(
          input_batch['ids'], normalize=False
      )
